...
from segearth_r2.mask_config.config import Config
from segearth_r2.datasets.dataset import *
from segearth_r2.datasets.dataset import LaSeRSDataset, EarthReasonDataset, UnifyDatasetSingleDatasetForBatch, DataCollatorForCOCODatasetV2
...
@dataclass
class DataArguments:
    data_path: str = field(default=None, metadata={"help": "Path to the dataset."})
    dataset_type: str = field(default="LaSeRS", metadata={"help": "Dataset type to train on."})
...
def smart_tokenizer_and_embedding_resize(
    ...
    model.resize_token_embeddings(max(len(tokenizer), model.config.vocab_size)) #Changed due to vocabulary shrinking
...
def make_unify_datamodule(clip_image_processor, tokenizer, data_args, training_args):
    ...
    if data_args.dataset_type == 'EarthReason':
        EarthReasonTrainDataset = EarthReasonDataset(base_data_path=data_args.base_data_path, tokenizer=tokenizer, data_args=data_args)
        datasets += [EarthReasonTrainDataset] * data_ratio[0]
    else:
        if data_ratio[0] != 0:
